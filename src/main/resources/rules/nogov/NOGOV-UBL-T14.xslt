...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="NOGOV T14 bound to UBL">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>NOGOV T14 bound to UBL</svrl:text>
...
<!--PATTERN UBL-T14-->
	...
	<!--RULE -->
<xsl:template match="//cac:CreditNoteLine" mode="M6" priority="1003">
    <svrl:fired-rule context="//cac:CreditNoteLine" />

		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="($Prerequisite1 and (cbc:CreditedQuantity/@unitCode != '')) or not ($Prerequisite1)" />
      <xsl:otherwise>
        <svrl:failed-assert test="($Prerequisite1 and (cbc:CreditedQuantity/@unitCode != '')) or not ($Prerequisite1)">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:Item" mode="M6" priority="1001">
